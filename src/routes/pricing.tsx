...
export const Route = createFileRoute("/pricing")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Transparent one-time pricing for funded trading challenges from $5K to $200K." },
      { property: "og:title", content: "FundedPlus Pricing" },
      { property: "og:description", content: "Simple one-time pricing. No subscriptions. No hidden fees." },
    ],
  }),
                ...
                <tr key={p.id} className="border-b border-border/30 last:border-0 hover:bg-card/30">
                  ...
                  <td className="p-4 text-right">
                    <Button asChild size="sm" className="bg-gradient-primary text-primary-foreground">
                      <Link to="/dashboard/checkout/$planId" params={{ planId: p.id }}>Buy</Link>
                    </Button>
                  </td>
                </tr>
              ))}
