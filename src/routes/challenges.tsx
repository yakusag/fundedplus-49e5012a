...
export const Route = createFileRoute("/challenges")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "2-step trading evaluation. Account sizes from $5K to $200K. Keep up to 90% of profits." },
      { property: "og:title", content: "FundedPlus Challenges" },
      { property: "og:description", content: "Pick your account size and start trading funded capital." },
    ],
  }),
  component: () => <SiteLayout><ChallengesPage /></SiteLayout>,
});
        ...
        {PLANS.map((p, i) => (
          ...
          >
            ...
            <Button asChild className="mt-6 bg-gradient-primary text-primary-foreground shadow-ice">
              <Link to="/dashboard/checkout/$planId" params={{ planId: p.id }}>Buy {p.label} challenge</Link>
            </Button>
          </motion.div>
        ))}
      </div>
